...
const setup = async () => {
    try {
        ...
        const superAdminUser = await accountsAPI.loginAdminByEmailAndPassword(userCredentials);
        ...
        const newUsers = await usersAPI.registerMultipleUsers(Array.from({ length: 3 }, (_, i) => ({
            name: `E2E Test User ${i + 1}`,
            email: `e2eTestUser${i + 1}@example.com`,
            organizationIDs: [organization.id],
            password: 'Test12345@',
        })));
        const groups = await Promise.all(Array.from({ length: 3 }, (_, i) => groupsAPI.registerGroup({
...
const teardown = async (organizationId) => {
    try {
        await accountsAPI.getAntiForgeryToken();
